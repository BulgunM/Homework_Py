...
shrub_n = int(input('Введите количество кустов: '))

list = []

for i in range(shrub_n):
    productivity = int(
        input('Введите количество ягод для каждого куста: '))
    list.append(productivity)

print(list)

max_count = 0

for i in range(1, shrub_n - 1):
    if (list[i] + list[i+1] + list[i-1] > max_count):
        max_count = list[i] + list[i+1] + list[i-1]
